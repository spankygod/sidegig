...
export default function GigsScreen() {
  ...
  const isInitialGigsLoading = isRefreshing && profile == null && myGigs.length === 0
  const hasError = error != null
  const refreshButtonLabel = getRefreshButtonLabel(isRefreshing)

  let gigsContent = myGigs.map((gig) => (
  ...
  if (myGigs.length === 0) {
    gigsContent = [
      <AppSurface key="empty-state" mode={mode}>
  ...
  return (
    ...
    >
      ...
      <PrimaryButton mode={mode} onPress={() => { router.navigate(authenticatedHomeHref) }} variant="secondary">
        Post another job
      </PrimaryButton>
